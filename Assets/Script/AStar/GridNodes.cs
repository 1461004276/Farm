...
namespace MFarm.AStar
{
    public class GridNodes
    {
        ...
        private int _width;
        private int _height;
        private Node[,] _gridNode; //二维数组来保存地图中的节点
        ...
        public GridNodes(int width, int height) //构造函数,初始化
        {
            ...
            for (int x = 0; x < width; x++)
            {
                ...
                {
                    _gridNode[x, y] = new Node(new Vector2Int(x, y)); //new Node时会自动调用Node中的构造函数将不同的x,y值传递给每一个节点
                }
            }
        }
        ...
        {
            if (xPos < _width && yPos < _height) //确保该坐标在地图范围之内
            {
