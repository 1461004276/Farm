...
public class AudioManager : Singleton<AudioManager>
{
    ...
    [Header("Audio Mixer")] public AudioMixer audioMixer; //Unity自带的音效混响组件的总控件,该组件可以方便的控制音效音乐的切换淡入淡出等待效果
    [Header("音响快照")] public AudioMixerSnapshot normalSnapshot; //AudioMixer组件下的快照属性,每一个单独的快照控制单独的音乐轨道,不同的音效效果Volume值大小等
    ...
    private void PlayMusicClip(SoundDetails soundDetails, float transitionTime)
    {
        audioMixer.SetFloat("MusicVolume",
            ConvertSoundVolume(soundDetails.soundVolume)); //为一个单独的轨道设定Volume值(float要转化为比特)
