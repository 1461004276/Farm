...
namespace MFarm.Inventory //手动添加一个命名空间，别的类不使用该命名空间就不可以调用到该命名空间中的变量或者函数，达到保护作用
{
    ...
    {
        [Header("物品数据")] 
        public ItemDataList_SO _itemDataList; //物品信息数据库
        [Header("建造蓝图")] 
        public BluePrintDataList_SO bulePrintData;
        [Header("背包数据")] 
        public InventoryBag_SO playerBagTemp;
        public InventoryBag_SO PlayerBag;// 玩家背包数据库
        private InventoryBag_SO currentBoxBag; //当前被开启箱子的数据库
        [Header("交易")] 
        ...
        private void OnStartNewGameEvent(int index)
        {
            ...
            boxDataDict.Clear();
            EventSystem.CallUpdateInventoryUI(InventoryFrom.Player, PlayerBag.itemList);
        }
        ...
        private void Start()
        {
/*            EventSystem.CallUpdateInventoryUI(InventoryFrom.Player, PlayerBag.itemList);//游戏一开始就调用一下更新UI的委托事件*/
        ...
        {
            return _itemDataList.itemDetailsList.Find(item => item.itemID == ID);
        }
        ...
        /// <param name="isDestroy">拾取后是否销毁物品</param>
        ...
        private void AddItemAtIndex(int ID, int index, int count)
        {
            if (index == -1 && CheckBagCapacity()) //背包中没有这个物体且背包中有空位
            {
                var item = new InventoryItem { itemID = ID, itemAmount = count }; //new一个在背包中的Item
                for (int i = 0; i < PlayerBag.itemList.Count; i++)
                {
            ...
            {
                int currentCount = PlayerBag.itemList[index].itemAmount + count; //获取背包中这个物体的当前数量
                var item = new InventoryItem { itemID = ID, itemAmount = currentCount }; //获得新物体的ID和Amount
        ...
        public void SwapItem(int fromIndex, int targetIndex)
        {
            ...
            EventSystem.CallUpdateInventoryUI(InventoryFrom.Player, PlayerBag.itemList); //刷新一下背包UI
        }

        /// <summary>
        /// 跨背包交换数据
        /// </summary>
        /// <param name="fromFrom"></param>
        /// <param name="fromIndex"></param>
        /// <param name="fromTarget"></param>
        /// <param name="targetIndex"></param>
        public void SwapItem(InventoryFrom fromFrom, int fromIndex, InventoryFrom fromTarget, int targetIndex)
        {
            var currentList = GetItemList(fromFrom);
            var targetList = GetItemList(fromTarget);
            ...
            if (targetIndex < targetList.Count)
            {
                ...
                EventSystem.CallUpdateInventoryUI(fromFrom, currentList);
                EventSystem.CallUpdateInventoryUI(fromTarget, targetList);
            }
        }
        ...
        /// <returns></returns>
        private List<InventoryItem> GetItemList(InventoryFrom from)
        {
            return from switch
            {
                InventoryFrom.Player => PlayerBag.itemList,
                InventoryFrom.Box => currentBoxBag.itemList,
                _ => null,
            };
        }
        ...
        private void RemoveItem(int ID, int removeAmount)
        {
            ...
            EventSystem.CallUpdateInventoryUI(InventoryFrom.Player, PlayerBag.itemList); //更新一下UI
        }
        ...
        public void RestoreData(GameSaveData saveDate)
        {
            ...
            EventSystem.CallUpdateInventoryUI(InventoryFrom.Player, PlayerBag.itemList); //刷新一下UI
        }
    }
}