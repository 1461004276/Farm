...
namespace Script.Utilities
{
    ...
    {
        // 定义道具栏UI更新事件
        // 需要知道更新格子的所属空间类型和更新的道具数据
        public static event Action<InventoryFrom, List<InventoryItem>> UpdateInventoryUI;
        public static void CallUpdateInventoryUI(InventoryFrom fromHere, List<InventoryItem> list)
        {
            UpdateInventoryUI?.Invoke(fromHere, list);
        }
